...
def recommend_sale_window(state, commodity, market, forecast_days=7):
    ...
    if days_to_peak > max_safe_hold_days:
        ...
        reason = (
            ...
            f"{expected_gain_pct:+.1f}% price gain. After accounting for "
            f"{cumulative_decay_pct:.1f}% estimated spoilage/holding cost "
            f"({daily_decay_pct}%/day x {days_to_peak} days), the net benefit is "
            f"{net_benefit_pct:+.1f}%. Holding is worthwhile."
        )
    else:
        decision = "SELL_NOW"
        if expected_gain_pct <= 0:
            ...
            reason = (
                f"The best predicted gain is {expected_gain_pct:+.1f}% on {peak_date}, "
                f"but after {cumulative_decay_pct:.1f}% spoilage/holding cost, the net "
                f"benefit is only {net_benefit_pct:+.1f}%. The expected gain doesn't "
    ...
    return {
        ...
        "backtested_mape": result["backtested_mape"],
        "state": state,
        "commodity": commodity,
        "market": market,
    }
